...
# ----------------------------------------------------------------------
# Frame façade — regroupe tous les outils de formation dans un
# sous-notebook, à intégrer comme un onglet de l'application principale.
# ----------------------------------------------------------------------
class FormationFrame(ttk.Frame):
    """Boîte à outils de formation (glossaire, calculateurs pédagogiques,
    quiz...) intégrable comme un onglet de l'application de gestion
    d'exploitation. N'écrit rien en base de données : usage pédagogique /
    de référence uniquement."""

    def __init__(self, parent):
        super().__init__(parent)
        avertissement = ttk.Label(
            self,
            text="Outils de formation et de référence — à usage pédagogique. Les "
                 "calculs de cet onglet sont indépendants des données réelles de "
                 "production (voir l'onglet « Bilan matière » pour les bilans calculés "
                 "à partir des relevés réels).",
            font=("Segoe UI", 9, "italic"), foreground="#555555", wraplength=820,
            padding=(10, 6),
        )
        avertissement.pack(fill="x")

        sous_notebook = ttk.Notebook(self)
        sous_notebook.pack(fill="both", expand=True)

        sous_notebook.add(OngletGlossaire(sous_notebook), text="Glossaire")
        sous_notebook.add(OngletBilan(sous_notebook), text="Calculateur bilan (simulation)")
        sous_notebook.add(OngletGranulo(sous_notebook), text="Granulométrie (P80)")
        sous_notebook.add(OngletSelection(sous_notebook), text="Choix de méthode")
